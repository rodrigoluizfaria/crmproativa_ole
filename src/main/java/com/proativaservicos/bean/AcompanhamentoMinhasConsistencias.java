...
@Named
@ViewScoped
public class AcompanhamentoMinhasConsistencias extends GenericBean implements Serializable {
    ...
    private List<?> listPendenciasFiltro;
    ...
    private void inicializarLojas() {
        ...
        if (this.usuario.getPerfil().equals(PerfilUsuarioEnum.SUPERVISOR)) {
            ...
            this.listLojas = this.serviceLoja
                    .pesquisarLojasPorEquipe(this.serviceUsuario.pesquisarEquipeUsuario(this.usuario.getId()));
    ...
    public void pesquisarMeusAgendamentos() {
        ...
        try {
            ...
            this.listHistoricosMeusAgendamentos = this.serviceHistoricoBko.pesquisarAgendamentos(this.usuario.getId(), null, null, this.cpf, this.dataInicioAgendamento, this.dataFimAgendamento, this.usuario, getEmpresa().getId(), true);
    ...
    public void onAtualizarPendentes() {
    ...
    public void pesquisarPendencias() {
        ...
        try {
            ...
            if (!this.usuario.getPerfil().equals(PerfilUsuarioEnum.SUPERVISOR))
                this.listPendencias = this.serviceAtendimentoBackoffice.pesquisarPendencias(null, this.operadorPendencia, this.usuario, retornarIdsLojas(), getEmpresa().getId(), null, null, null, false);
    ...
    public void pesquisarMinhaLista() {
        ...
        try {
            ...
            this.listMinhasPendencias = this.serviceAtendimentoBackoffice.pesquisarMinhaListaAtendimento(this.usuario.getId(), this.cpf, this.adesao, this.nome, this.consistencia, (this.status == null ? null : this.status.getId()), this.produto, this.loja, (this.motivo == null ? null : this.motivo.getId()), (this.subMotivo == null ? null : this.subMotivo.getId()), this.dataInicio, this.dataFim);
    ...
    public boolean validarUsuarioAtendimento(AtendimentoBackoffice atendimento) {
        ...
        } else if (atendimento.getUsuarioEmAtendimento().getId() == this.usuario.getId()) {
    ...
    private void pesquisarQuantidadeConsistencia() {
        ...
        this.listQuantidadeConsistencia = this.serviceAtendimentoBackoffice.pesquisarQuantidadeConsistencia(retornarUsuarioSessao().getId(), this.dataInicioPanel, this.dataFimPanel);

        this.listProdutividade = null;
        Long usr[] = {this.retornarUsuarioSessao().getId()};

        this.listProdutividade = this.serviceAtendimentoBackoffice.pesquisarProdutividadeAtendimento(null, usr, null, null, retornarUsuarioSessao(), this.dataInicioPanel, this.dataFimPanel, TipoVisualizacaoEnum.USUARIO);

        criarDashConsistencia();
        criarProdutividadeObject();
    }

    private void criarProdutividadeObject() {

        if (CollectionUtils.isNotEmpty(this.listProdutividade)) {

            this.produtividade = (Object[]) this.listProdutividade.get(0);
        }
    }
    ...
    public boolean globalFilterFunction(Object value, Object filter, Locale locale) {
        ...
        return (ob[1] != null && ob[1].toString().toLowerCase().startsWith(filterText))
                || (ob[2] != null && ob[2].toString().toLowerCase().startsWith(filterText))
                || (ob[3] != null && ob[3].toString().toLowerCase().startsWith(filterText))
                || (ob[7] != null && ob[7].toString().toLowerCase().startsWith(filterText))
                || (ob[4] != null && ob[4].toString().toLowerCase().startsWith(filterText))
                || (ob[6] != null && ob[6].toString().toLowerCase().startsWith(filterText));
    ...
    public List<?> getListPendenciasFiltro() {
        return listPendenciasFiltro;
    }

    public void setListPendenciasFiltro(List<?> listPendenciasFiltro) {
        this.listPendenciasFiltro = listPendenciasFiltro;
    }
}
