...
public class Usuario extends GenericControle implements Serializable {
    ...
    public String getNomeSobrenome() {
        ...
        String[] arrayNome = this.nome.trim().split("\\s+");
        StringBuilder builder = new StringBuilder();

        if (arrayNome.length > 1) {


            builder.append(Character.toUpperCase(arrayNome[0].charAt(0)))
                    .append(arrayNome[0].substring(1).toLowerCase())
                    .append(" ");

            String ultimo = arrayNome[arrayNome.length - 1];

            builder.append(Character.toUpperCase(ultimo.charAt(0)))
                    .append(ultimo.substring(1).toLowerCase());

            return builder.toString();
        }


        return Character.toUpperCase(this.nome.charAt(0))
